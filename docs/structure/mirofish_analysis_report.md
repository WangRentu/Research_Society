...
| 维度 | MiroFish | research_society (aira-cogito + airs-bench) |
|------|----------|------------------------------------------|
...
**问题**: aira-cogito 的 Agent 没有跨步记忆，导致反复犯同样的错误（buggy 率 ~90%）。

**方案**: 将 MiroFish 的 Zep 记忆机制引入 aira-cogito。
...
**实现路径**: aira-cogito 已有 `memory_op` 接口（draft/debug/improve 算子均支持），只需接入 Zep GraphRAG 替换当前简单实现。
...
2. **执行阶段**: 将讨论共识转化为 Draft，用搜索策略优化（借鉴 aira-cogito）
