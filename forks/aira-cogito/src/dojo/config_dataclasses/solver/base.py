...
def _default_available_packages() -> list[str]:
    """Auto-generate from superimage/pip.requirements.txt when available."""
    from dojo.utils.environment import get_packages_from_requirements

    pkgs = get_packages_from_requirements()
    return pkgs if pkgs else list(_FALLBACK_PACKAGES)
...
class SolverConfig(BaseConfig):
    ...
    available_packages: list[str] = field(
        default_factory=_default_available_packages,
        metadata={
            "description": "List of available packages for the solver. "
            "Auto-generated from superimage/pip.requirements.txt when present.",
