...
def launch_jobs(config_list: list[RunConfig], launcher_cfg: LauncherConfig):
    ...
    snapshot_path = Path(get_log_dir()) / "aira-cogito" / "snapshots" / f"{date_str}"
    ...
    with RsyncSnapshot(
        ...
        include=glob.glob("./src/dojo/tasks/**", recursive=True),
    ):
        ...
        # because of the editable installation of aira-cogito and aira-core

        # Note 3: PYTHONPATH parses multiple paths given with `:` char
        # We bypass the editable installs with snapshot copies.
        ...
        os.environ["PYTHONPATH"] = (
            f"{snapshot_path}/src:{snapshot_path}/aira-core/src"
        )
