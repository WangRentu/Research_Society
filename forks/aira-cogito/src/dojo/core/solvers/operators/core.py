...
def execute_op_plan_code(
    operator_fn: Callable, *operator_args, max_operator_tries: int, requires_plan: bool = False, **operator_kwargs
) -> tuple[str, str, str]:
    ...
    for _ in range(max_operator_tries):
        completion_text, metrics = operator_fn(*operator_args, **operator_kwargs)
