...
def _normalize_json_like_function_arguments(text: str) -> str:
    """Best-effort normalization for tool/function-call arguments.

    Some OpenAI-compatible backends occasionally emit Python literals (None/True/False)
    in the `function_call.arguments` string, which is expected to be strict JSON.

    We only rewrite tokens that appear *outside* JSON strings to avoid corrupting
    user-facing text like summaries.
    """
    ...
    def _query_client(
        ...
        for attempt in range(max_connection_retries):
            try:
                ...
                logger.warning(
                    f"LLM connection error (attempt {attempt+1}/{max_connection_retries}): "
                    f"{type(e).__name__}: {str(e)[:200]}. Retrying in {wait}s..."
                )
                if attempt < max_connection_retries - 1:
                    time.sleep(wait)
                else:
                    logger.error(f"LLM connection failed after {max_connection_retries} attempts, raising.")
                    raise
            ...
            if not function_call:
                ...
                if not str(function_call.name).strip() == str(func_spec.name).strip():
                    ...
                    try:
                        ...
                        # Some backends emit Python literals (None/True/False) in arguments.
                        # Normalize outside-string tokens and retry.
                        normalized = _normalize_json_like_function_arguments(function_call.arguments)
                        try:
                            ...
                            logger.error(
                                "Error decoding normalized function arguments:\n" + str(normalized)
                            )
                            raise ex
