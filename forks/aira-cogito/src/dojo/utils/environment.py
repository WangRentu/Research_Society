...
def parse_pip_list_output(pip_output: str, include_versions: bool = True) -> list[str]:
    """Parse the stdout of ``pip list --format=freeze`` into package specs.

    Parameters
    ----------
    pip_output : str
        Raw output from ``pip list --format=freeze``.
    include_versions : bool
        If *True* (default), return ``"package==version"`` strings so the
        LLM knows which API surface is available.  If *False*, return bare
        package names (legacy behaviour).
    """
    packages: list[str] = []
    for line in pip_output.splitlines():
        ...
        if "==" in line:
            if include_versions:
                # keep full "package==version" string
