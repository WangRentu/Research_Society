...
This will create a new conda environment named `aira-cogito` with all the required packages.

## Step 3. Install aira-cogito via pip
```bash
pip install -e .
```
...
1. **Conda environment creation fails**
   ...
   - Try creating the environment with specific Python version: `conda create -n aira-cogito python=3.11`
...
2. **Package import errors after installation**
   - Ensure the conda environment is activated: `conda activate aira-cogito`
   - Verify the package was installed correctly: `pip list | grep aira-cogito`
